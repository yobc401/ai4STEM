...
def get_boxes_from_xyz(filename, frame_size, sliding_volume, stride_size, adapt=True, element_agnostic=False,
    ...
    x_max = frame_size[0] #max(x) + padding[0]
    x_min = 0.0 #min(x) - padding[0]
    y_max = frame_size[1] #max(y) + padding[1]
    y_min = 0.0 #min(y) - padding[1]
    z_max = frame_size[2] #max(z) + padding[2]
    z_min = 0.0 #min(z) - padding[2]
        ...
        for i in range(number_of_strides_vector[1]):
            ...
            for j in range(number_of_strides_vector[0]):
                ...
                for vector, element_name in coordinate_vector_and_element_list:
                    ...
                    if condition:
                        positionvectors_within_sliding_volume.append([element_name,vector])
                ...
                if len(positionvectors_within_sliding_volume) == 0:
                    ...
                    list_of_x_boxes.append(positionvectors_within_sliding_volume)
